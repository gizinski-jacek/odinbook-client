...
const Navbar = () => {
	...
	// 	(async () => {
	// 		try {
	// 			setShowNotificationsRequests(await axiosGet('/api/users/requests'));
	// 		} catch (error: any) {
	// 			console.error(error);
	// 		}
	// 	})();
	...
	const toggleMenu = (e: React.MouseEvent<HTMLLIElement>) => {
		e.stopPropagation();
	...
	const notificationsRequestsDisplay = showNotificationsRequests?.map(
		(request) => {
			return;
			// return (
			// 	<RequestWrapper
