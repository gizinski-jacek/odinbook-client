...
const Navbar = () => {
	...
	const toggleMenu = (
		...
		if (openMenuContainer && openMenu === menuNumber) {
			setOpenMenuContainer(false);
			setOpenMenu(0);
		...
		<nav className={styles.navbar}>
			...
			<div className={styles.right}>
				<ul className={styles.user_controls}>
					...
					>
						<span
							className={`${styles.icon} ${openMenu === 1 ? 'btn-active' : ''}`}
						>
					...
					>
						<span
							className={`${styles.icon} ${openMenu === 2 ? 'btn-active' : ''}`}
						>
					...
					>
						<span
							className={`${styles.icon} ${openMenu === 3 ? 'btn-active' : ''}`}
						>
							<svg viewBox='0 0 28 28' height='20' width='20'>
					...
					>
						<span
							className={`${styles.icon} ${openMenu === 4 ? 'btn-active' : ''}`}
						>
				...
				{!openMenuContainer ? null : (
					<div ref={menuContainerRef} className={styles.menu_container}>
						{openMenu === 1 ? <AccountMenu /> : null}
						{openMenu === 2 ? (
							<NotificationsMenu setNotificationAlert={setNotificationAlert} />
						) : null}
						{openMenu === 3 ? <MessengerMenu /> : null}
						{openMenu === 4 ? <MainMenu /> : null}
					</div>
				)}
			</div>
		</nav>
